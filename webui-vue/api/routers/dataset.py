...
@router.get("/adaptive-crop/status")
async def get_adaptive_crop_status():
    return adaptive_crop_state

@router.post("/adaptive-crop/stop")
async def stop_adaptive_crop():
    adaptive_crop_state["running"] = False
    return {"success": True}

@router.post("/adaptive-crop")
async def adaptive_crop_images(request: AdaptiveCropRequest):
    """适应性裁剪图片以减少分桶丢弃"""
    global adaptive_crop_state
    
    if adaptive_crop_state["running"]:
        raise HTTPException(status_code=400, detail="裁剪任务正在进行中")
    
    dataset_path = Path(request.dataset_path)
    if not dataset_path.exists():
        raise HTTPException(status_code=404, detail="数据集路径不存在")
    
    limit = request.resolution_limit
    ...
    if not image_paths:
        return {"success": True, "message": "没有找到图片", "total": 0}
    
    def get_bucket_size(w: int, h: int) -> tuple:
        if max(w, h) > limit:
            scale = limit / max(w, h)
            w = int(w * scale)
            h = int(h * scale)
    ...
    for img_path in image_paths:
        try:
            with Image.open(img_path) as img:
                ow, oh = img.size
            bw, bh = get_bucket_size(ow, oh)
            key = f"{bw}x{bh}"
            if key not in buckets:
                buckets[key] = {"width": bw, "height": bh, "count": 0, "images": []}
            buckets[key]["count"] += 1
            buckets[key]["images"].append({"path": img_path, "orig_w": ow, "orig_h": oh})
        except:
            pass
    
    sorted_keys = sorted(buckets.keys(), key=lambda k: buckets[k]["count"], reverse=True)
    merge_plan = {}
    
    for src_key in sorted_keys:
        src = buckets[src_key]
        src_batches = src["count"] // batch_size
        src_remainder = src["count"] % batch_size
        if src_remainder == 0 or src_batches == 0:
            continue
        
        for dst_key in sorted_keys:
            if src_key == dst_key:
                continue
            dst = buckets[dst_key]
            if dst["count"] // batch_size == 0:
                continue
            
            w_diff = abs(src["width"] - dst["width"])
            h_diff = abs(src["height"] - dst["height"])
            if w_diff > tolerance or h_diff > tolerance:
                continue
            
            src_ratio = src["width"] / src["height"]
            dst_ratio = dst["width"] / dst["height"]
            if abs(src_ratio - dst_ratio) > 0.1:
                continue
            
            old_dropped = src_remainder + (dst["count"] % batch_size)
            new_dst_count = dst["count"] + src_remainder
            new_src_count = src["count"] - src_remainder
            new_dropped = (new_dst_count % batch_size) + (new_src_count % batch_size if new_src_count > 0 else 0)
            
            if new_dropped < old_dropped:
                merge_plan[src_key] = {"target_w": dst["width"], "target_h": dst["height"], "count": src_remainder}
                break
    
    if not merge_plan:
        return {"success": True, "message": "当前分桶已最优，无需裁剪", "total": 0, "cropped": 0}
    
    total_to_crop = sum(p["count"] for p in merge_plan.values())
    
    adaptive_crop_state.update({
        "running": True, "total": total_to_crop, "completed": 0,
        "current_file": "", "cropped": 0, "skipped": 0
    })
    
    async def run_crop():
        global adaptive_crop_state
        for src_key, plan in merge_plan.items():
            if not adaptive_crop_state["running"]:
                break
            images_to_crop = buckets[src_key]["images"][-plan["count"]:]
            target_w, target_h = plan["target_w"], plan["target_h"]
            
            for img_info in images_to_crop:
                if not adaptive_crop_state["running"]:
                    break
                img_path = img_info["path"]
                adaptive_crop_state["current_file"] = img_path.name
                
                try:
                    with Image.open(img_path) as img:
                        ow, oh = img.size
                        scale = min(1.0, limit / max(ow, oh))
                        sw, sh = int(ow * scale), int(oh * scale)
                        crop_w = max(0, sw - target_w)
                        crop_h = max(0, sh - target_h)
                        
                        if crop_w == 0 and crop_h == 0:
                            adaptive_crop_state["skipped"] += 1
                        else:
                            left = int((crop_w / 2) / scale)
                            top = int((crop_h / 2) / scale)
                            right = ow - int((crop_w - crop_w // 2) / scale)
                            bottom = oh - int((crop_h - crop_h // 2) / scale)
                            ...
                            if img_path.suffix.lower() in ['.jpg', '.jpeg']:
                                ...
                                kwargs = {'quality': 95}
                            cropped.save(img_path, **kwargs)
                            adaptive_crop_state["cropped"] += 1
                except Exception as e:
                    print(f"[AdaptiveCrop] Error: {img_path}: {e}")
                    adaptive_crop_state["skipped"] += 1
                
                adaptive_crop_state["completed"] += 1
                await asyncio.sleep(0.01)
        
        adaptive_crop_state["running"] = False
        adaptive_crop_state["current_file"] = ""
    
    asyncio.create_task(run_crop())
    return {"success": True, "message": f"开始裁剪 {total_to_crop} 张图片", "total": total_to_crop}